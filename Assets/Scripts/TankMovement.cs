...
public class TankMovement
{
    ...
    public TankMovement(TankModel tankModel, TankView tankPrefab)
    {
        TankModel = tankModel;
    ...
    public void HandleLeftJoyStickInput(Rigidbody tankRigidBody)
    {
        ...
        {
            Vector3 ZAxisMovement = tankRigidBody.transform.position + (tankRigidBody.transform.forward * variableJoystick.Vertical * TankModel.Speed * SpeedMultipier);
            tankRigidBody.MovePosition(ZAxisMovement);
        }

        if (variableJoystick.Horizontal != 0)
        {
            Quaternion newRotation = tankRigidBody.transform.rotation * Quaternion.Euler(Vector3.up * variableJoystick.Horizontal * TankModel.RotationSpeed * RotationSpeedMultiplier);
    ...
    {
        Vector3 desiredRotation = Vector3.up * variableJoystick.Horizontal * TankModel.TurretRotationSpeed * RotationSpeedMultiplier;
